 ...
 * for one accent, colours are stored on the record as plain hex and applied
 ...
 * That matters here because the app ships both themes and a tab colour has to
 ...
 *
 * Anything else — a colour from a future picker, a corrupted row, a synced
 * value from a client that stored oklch — is rejected rather than concatenated
 * into a string that is not a colour at all. `#rgb` shorthand is excluded on
 * purpose: `#abc20` is not a valid colour.
 */
export function isTintableColor(color: string | null | undefined): color is string {
  return typeof color === "string" && /^#[0-9a-f]{6}$/i.test(color);
}

/** A faint wash of the colour, for fills behind text. */
