...
import { internalOnlyPaths, labelAndTabItemPaths, listItemPath, patchTodoPath, v1Paths } from "./routes";
      ...
      securitySchemes: {
        ...
        bearerAuth: {
          ...
          description:
            ...
            "handoff (/api/desktop/exchange). Scoped per key (A2, EI-227) " +
            "— see src/server/auth-scopes.ts.",
        },
      },
...
/**
 * The public document. `/api/v1` reads landed in A2 (EI-227), writes in A5
 * (EI-230). Published by A7 (EI-231).
 */
export function buildPublicDocument() {
  return createDocument({
    ...
    info: {
      ...
      version: "0.1.0",
      description:
        "The public, versioned Faite API. Read and write endpoints for " +
        "todos, gated by API key scope (A2, EI-227; A5, EI-230). " +
        "Generated by scripts/openapi/generate.ts — do not hand-edit.",
    },
    /**
     * An ABSOLUTE server, not the implied relative `/` (EI-307).
     *
     * OpenAPI treats an absent `servers` as a single server at `/`, resolved
     * against wherever the document is served from. That happens to work on
     * `https://myfaite.app/docs` — same origin, so Scalar's "Test Request"
     * button reaches the real API and never even triggers a CORS preflight.
     *
     * It breaks everywhere else the same page ships. `/docs` is part of the
     * static export too (`BUILD_TARGET=static`, ARCHITECTURE §2.12), where
     * the page is served from `capacitor://localhost` and a relative
     * `/api/v1/todos` resolves against THAT origin — a host with no API on
     * it. Naming the host explicitly is also what makes the try-it panel
     * honest for anyone reading the docs from a local `next dev` on :3000.
     *
     * Those non-myfaite origins ARE cross-origin, which is what makes the
     * preflight's `Access-Control-Allow-Methods` list load-bearing for this
     * page rather than merely correct — see `cors.ts`.
     */
    servers: [{ url: "https://myfaite.app", description: "Production" }],
    security: [{ bearerAuth: [] }],
    components: {
      securitySchemes: {
        bearerAuth: {
    ...
    paths: { ...v1Paths, ...patchTodoPath, ...listItemPath, ...labelAndTabItemPaths },
  });
}
