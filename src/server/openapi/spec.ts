...
import { internalOnlyPaths, v1Paths } from "./routes";
...
/**
 * The public document. `/api/v1` reads landed in A2 (EI-227); writes are
 * A5's job. Published by A7 (EI-231).
 */
export function buildPublicDocument() {
  return createDocument({
    ...
    info: {
      ...
      version: "0.1.0",
      description:
        "The public, versioned Faite API. Read-only for now (A2, EI-227) — " +
        "write endpoints land in A5 (EI-230). Published by A7 (EI-231). " +
