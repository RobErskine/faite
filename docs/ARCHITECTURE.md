...
delivered to a handler export on the Worker entry, and this file *is* the
Worker entry — so receiving mail needed no new Worker, no new deploy target,
and nothing added to `wrangler.jsonc` beyond a `vars` entry naming the domain
(the catch-all rule itself is zone configuration). It is also the first real
consumer of the transport-agnostic service layer from EI-50: it builds a
...
src/
  ...
  components/
    ...
    todo/                     capture-source-badge.tsx — where a to-do came FROM
    ui/                       shadcn components
  server/
    worker.ts                 custom worker entry (exports the DO, mounts auth + email())
    ...
    email.ts                  sendEmail() over the EMAIL binding — OUTBOUND
    email/                    INBOUND ingest (EI-186) — see docs/EMAIL-INGEST.md
      ingest.ts               email() body: guard → resolve → rate → parse → push
      parse.ts                pure: parsed message → CreateTodoInput
