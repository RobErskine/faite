...
| `todo-sheet.tsx` | `Enter` blurs the title (commit-on-blur does the write); on `SheetContent`, bare `↑` marks done and bare `←` marks won't-do whenever focus is not in a text field — the **same two keys, same meanings** as `overdrive-overlay.tsx` below, so triage is one set of muscle memory rather than two (EI-318); `↓` and `→` stay unbound, because Overdrive's meanings for them need its verdict state machine and inventing different ones here would break exactly the memory the other two exist to share. This is also why the sheet does **not** autofocus its title on open — the caret sitting in a textarea would make both arrows dead on arrival. Also `⌘Enter`/`Ctrl+Enter` marks done, `⌘⌫`/`Ctrl+Backspace` marks won't-do, `⇧⌘⌫`/`Ctrl+Shift+Backspace` deletes — local rather than global because these are meaningless with no sheet open, and the registry's `GuardContext` has no per-surface discriminator (§4.3) to scope a global entry to just this sheet; the Sub-tasks section's "Add a sub-task" input (EI-55) is the same commit-on-`Enter`-or-blur pattern as the title field, scoped to that one input |
| `todo-card-menu.tsx` | With the card's context menu open: `⌘↵`/`Ctrl+Enter` marks done (or reopens), `⌘⌫`/`Ctrl+Backspace` marks won't do, `⇧⌘⌫`/`Ctrl+Shift+Backspace` deletes — the **same three chords, same meanings** as `todo-sheet.tsx` and the ⌘K palette, so the menu reinforces them rather than inventing any (EI-289). Local rather than global for the same reason the sheet's are: meaningless with no menu open, and while one IS open `contextMenuOpen` holds every board hotkey off, so nothing else could catch them. Modifiers only — Base UI's Menu owns bare letters via `useTypeahead`. The chord is part of each item's accessible name, so it is announced, not just drawn |
| `todo-card.tsx` | `←→↑↓` navigate (§11), `Enter` opens the sheet, `Space` toggles done, `x` adds/removes the row from the multi-selection (EI-194 — a letter, because `Space` is already spoken for); dnd-kit's keyboard drag activator, on the grip; the **Menu key** opens the row's context menu (EI-285) — not bound by this app at all, the platform turns it into a `contextmenu` event and Base UI's trigger answers it, which is why it is reached from the *title button* (a real tab stop) rather than the `tabIndex={-1}` row. `Shift+F10` is the same gesture on Windows hardware; Chromium does not synthesize it, so only the Menu key is asserted in e2e. See docs/CONTEXT-MENU.md |
...
| `overdrive-overlay.tsx` | `←` won't-does the current card, but is stage-aware first: it unstages a picked date, then walks a ramp down, and only commits "won't do" once nothing is staged. `↑` done, `↓` back-to-list, `⇧↓` forces Backlog, `→`/`⇧→` stage a schedule ramp (day/week) — counting from today for an Overflow session, or from the day the session was entered from for a day session (EI-253; the ramp's floor there is 1, not 0, so the first press never re-stages the day the card is already on — see `docs/OVERDRIVE.md` §5a), `Enter` confirms it (or, on the finish screen, closes the overlay — `confirm` is dead there), `D` opens the date picker, `⌫` steps back one verdict (works from the finish screen too), local `⌘Z`/`Ctrl+Z` also steps back (the global `mod+z` is blocked here by `overdriveSource` in `computeModalOpen`) — local for the same reason `todo-sheet.tsx` is: meaningless with the overlay closed, and every key here is contextual (an arrow means something completely different on the board underneath). See `docs/OVERDRIVE.md` §9. `Escape` is intercepted one level up, at the **`Dialog`**'s `onOpenChange` (`eventDetails.reason === "escape-key"`), so a staged day can be cleared by the first press instead of Base UI closing the whole overlay on it — the same override point `command-palette.tsx`'s sub-modes would need if they ever wanted to catch Escape before the palette itself closes. |
