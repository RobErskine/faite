...
Read D0's finding first if you haven't: `docs/DESKTOP.md` §3.4 (on
`rob/d0-desktop-spike`, not merged to `main`) — a hidden (`visible: false`)
`WebviewWindow`'s JS `setInterval` fires once and then never again, killing
the planned design where a hidden background webview runs the sync engine's
~30s poll loop while the main board window is closed.
...
## 6. Files this spike added/changed
...
- `src-tauri/src/ei178_probe.rs` — the probe harness described in §1.
  ...
  non-probe build path (same convention as D0's `d0_probe.rs`, which this
  file replaces on this branch — this branch was cut from `main`, not from
  `rob/d0-desktop-spike`, so it doesn't carry D0's Next.js-export plumbing;
  see §1 for why a plain static HTML file was enough here).
...
- `src-tauri/{Cargo.lock,build.rs,capabilities/,icons/,.gitignore}`,
  ...
  D0's already-reviewed scaffold (`rob/d0-desktop-spike`) rather than
  re-deriving them.
