...
| [HOMEPAGE.md](HOMEPAGE.md) | The `/` story: the beat table everything derives from, the zero-client-JS hero board, the traveling card, the ticks, and the known limits. |
| [SCENE.md](SCENE.md) | The 3D room on the homepage: the bake pipeline, placement in meters, token re-tinting, the load invariants, what the room does during each beat, and the CC-BY chain. |
