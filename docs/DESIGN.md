...
| 2026-09-10 | **Priority gains a third channel: rhythm.** Solid, long dash, short dash, sparse dots for P1–P4, alongside the existing thickness and opacity. | A width ramp answers "which of these two is higher" but never "what is this one" — a level had to be compared against a neighbour to be read. Rhythm is identifiable alone, is still form rather than color (so decision A stands untouched), and does not suffer the device-pixel rounding that killed a four-step width ramp. Every period is 9px or under so at least three marks land in a card rail's ~27px; `priority.test.ts` holds that ceiling and caught a 12px P2 that read as solid. |
